...
def high_pass_filter(img, kernel_size=3):
    ...
    # 对图像应用卷积操作
    filtered_img = Func.conv2d(img, kernel, padding=kernel_size // 2, groups=img.size(1))
...
def ugc_sr_base(args):
    ...
    for image_name in image_names:
        ...
        # Step 2: Fuse PSNR img and IQA img with 
        print("Image Fusion")
        transform = ToTensor()
        ...
        fusion_img = fusion_image_psnr + high_freq_iqa
        fusion_img = torch.clamp(fusion_img, 0, 1)
        fusion_img.requires_grad_(False)
